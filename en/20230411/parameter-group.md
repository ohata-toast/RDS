...
DB 인스턴스에 설치된 MySQL의 설정을 적용하기 위해서 RDS for MySQL은 파라미터 그룹 기능을 제공합니다. 파라미터 그룹은 MySQL을 설정할 수 있는 파라미터의 집합입니다. 서비스 활성화 시, 모든 DB 엔진 버전별로 기본 파라미터 그룹을 제공합니다. 기본 파라미터 그룹은 `default.{DB 엔진 버전 이름}` 으로 제공되며, 버전별 권장하는 기본 파라미터값으로 구성되어 있습니다. 기본 파라미터 그룹은 일반 파라미터 그룹과 동일하게 수정, 삭제할 수 있습니다.

### 파라미터 그룹 생성

필요에 따라 파라미터 웹콘솔에서 파라미터 그룹을 생성할 수 있습니다. 파라미터 그룹은 DB 엔진 버전별로 생성하며 이름을 부여할 수 있으며 아래와 같은 제약사항이 있습니다.

* 파라미터 그룹 이름은 리전별로 고유해야 합니다.
* 파라미터 그룹 이름은 1 ~ 100 사이의 알파벳, 숫자, - _ . 만 사용할 수 있으며, 첫 번째 문자는 알파벳이어야 합니다.
...
웹콘솔에서 서로 다른 2개의 파라미터 그룹을 선택한 후, **비교** 버튼을 클릭하면 파라미터가 무엇이 다른지 확인할 수 있습니다. 동일한 DB 엔진 버전뿐만 아니라 서로 다른 DB 엔진 버전의 파라미터 그룹도 비교할 수 있습니다.
...
웹콘솔에서 파라미터 그룹을 선택한 후, **파라미터 편집** 버튼을 누르면 파라미터를 변경할 수 있습니다. 변경할 수 없는 파라미터는 값이 일반 텍스트로 나타나며, 변경할 수 있는 파라미터는 값을 변경할 수 있는 INPUT이 나타납니다. 편집 화면에서 `변경 사항 미리 보기` 버튼을 누르면 변경된 파라미터를 확인할 수 있는 별도 팝업 화면이 표시되며, `재설정` 버튼을 누르면 변경하기 전으로 되돌아갈 수 있습니다. 편집 모드에서 변경한 모든 값은 `변경 사항 저장` 버튼을 눌러야 파라미터 그룹에 반영됩니다. 변경된 파라미터 그룹의 DB 인스턴스 반영은 [파라미터 그룹 적용](./#_5) 항목을 참고합니다.