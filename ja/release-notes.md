...
* Open API を通じてイベントサブスクリプションを作成する際に、イベントソースに異常値を入力できる問題を修正しました
* 断続的にDBインスタンスのステータスが更新されない問題を修正しました
* 断続的に不明なエラーモーダルウィンドウが表示される問題を修正しました
...
* 特定の条件でモニタリングデータが収集されない現象を修正しました
* フェイルオーバーが発生したインスタンスの自動バックアップが削除されない現象を修正しました
* 作成に失敗した自動バックアップが期間満了時に削除されない現象を修正しました
* MySQLに登録されたユーザーが数が多すぎる場合に復元に失敗する現象を修正しました
* access ruleを修正してもバックアップ設定変更イベントが記録される現象を修正しました
