...
* 고가용성 기능이 일지 중지되어도 데이터 복제는 정상적으로 이루어지나, 장애가 감지되지 않기 때문에 장시간 일시 정지 상태로 유지하는 것을 권장하지 않습니다.
...
* NHN Cloud Cloud Compute & Network 상품에서 제공하는 일부 사양으로 DB 인스턴스를 생성할 수 있습니다.
...
* NHN Cloud Cloud RDS 는 시점 복원을 지원합니다.
