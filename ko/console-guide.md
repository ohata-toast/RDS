...
![rds_02_20210112](https://static.toastoven.net/prod_rds/21.09.14/rds_01_20210914.png)
...
* 고가용성 인스턴스를 재시작할 때 [장애 조치를 이용한 재시작] 옵션을 선택해 Master와 Candidate Master를 교체할 수 있습니다.
  ...
  * Master에 lock이 있거나 복제 딜레이가 길 경우 장애 조치가 되지 않거나 시간이 많이 소요되므로, 트랜잭션이 모두 종료되었고 복제 딜레이가 없는지 확인해야 합니다.
...
* 대상 DB 인스턴스가 재시작 됩니다.
* 고가용성 인스턴스일 경우 장애 조치를 이용한 재시작을 할 수 있습니다.
  * 장애 조치를 이용하여 Storage를 확장할 경우 마스터가 교체됩니다. 순단 시간은 줄어들지만 전체 작업 시간은 조금 더 시간이 걸릴 수 있습니다.
  * 마스터가 교체되면 bin log 파일이 모두 삭제되며, 교체되기 전 시간으로 시점 복원이 불가능합니다.
  * Master에 lock이 있거나 복제 딜레이가 길 경우 장애 조치가 되지 않거나 시간이 많이 소요되므로, 트랜잭션이 모두 종료되었고 복제 딜레이가 없는지 확인해야 합니다.
