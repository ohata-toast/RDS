...
고가용성 구성을 더 이상 사용하지 않을 경우 장애 조치된 마스터를 분리해야 합니다. 분리하면 장애 조치된 마스터는 새로운 복제 그룹을 형성하고, DB 인스턴스의 종류가 마스터로 변경됩니다. 분리된 마스터와 승격된 마스터 사이의 모든 관계는 단절되며, 더 이상 승격된 마스터와 고가용성 구성으로 묶일 수 없습니다. 분리되어 새로운 마스터가 된 이후부터 해당 NHN Cloud가 제공하는 모든 기능을 다시 사용할 수 있습니다.
