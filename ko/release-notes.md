...
* 다른 서비스와 유사한 룩엔필의 UI로 개선되었습니다.
* DB 인스턴스가 삭제되도 수동 백업은 삭제되지 않도록 수정되었습니다.
* 파라미터 그룹 기능이 추가되었습니다.
  * 파라미터 그룹을 이용하여 DB 인스턴스의 데이터베이스 설정을 자유롭게 변경할 수 있습니다.
  * 파라미터 그룹은 여러 DB 인스턴스에 적용할 수 있습니다. 
  * 기존 DB 인스턴스의 설정 변경 사항은 DB 인스턴스와 동일한 이름의 파라미터 그룹으로 마이그레이션 되었습니다.
* DB 보안 그룹 기능 추가
  * DB 보안 그룹을 이용하여 DB 인스턴스의 접근 제어를 자유롭게 설정할 수 있습니다.
  * DB 보안 그룹은 여러 DB 인스턴스에 적용할 수 있습니다.
  * 기존 DB 인스턴스에 설정된 접근 제어 규칙은 `{DB 인스턴스 이름}__{DB 인스턴스 아이디}`규칙으로 명명된 DB 보안 그룹으로 마이그레이션 되었습니다.
* 복제 관계에 따라 DB 인스턴스를 묶어서 볼 수 있는 화면이 제공됩니다.
* 예비 마스터가 웹 콘솔에 노출됩니다.
  * 예비 마스터의 바이너리 로그(binary log)를 삭제하여 용량 확보를 할 수 있습니다.
  * 예비 마스터의 각종 로그를 보거나 다운로드 받을 수 있습니다.
  * 예비 마스터에 문제가 발생한 경우, 재구축할 수 있습니다.
    * 예비 마스터의 DB 인스턴스는 변경되지 않아, fixed ip주소가 변경되지 않습니다.
    * 데이터베이스의 데이터는 모두 삭제되며, 마스터의 데이터로 복구됩니다.
* 읽기 복제본을 재구축 할 수 있습니다.
  * 읽기 복제본의 DB 인스턴스는 변경되지 않아, fixed ip주소가 변경되지 않습니다.
  * 데이터베이스의 데이터는 모두 삭제되며, 마스터의 데이터로 복구됩니다.
* 장애 조치된 마스터를 복구할 수 있습니다.
  * 장애 조치된 마스터와 신규 마스터를 다시 고가용성 구성으로 복구 할 수 있습니다.
  * 복구는 실패할 수 있으며, 복구 할 수 없는 장애 조치된 마스터는 재구축 할 수 있습니다.
* 장애 조치된 마스터를 재구축 할 수 있습니다.
  * 장애 조치된 마스터의 DB 인스턴스는 변경되지 않아, fixed ip주소가 변경되지 않습니다.
  * 데이터베이스의 데이터는 모두 삭제되며, 마스터의 데이터로 복구됩니다.
