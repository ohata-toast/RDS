...
外部MariaDBのバックアップファイルをNHN Cloudのユーザーオブジェクトストレージにアップロードして、RDS for MariaDBのDBインスタンスに復元することができます。詳細は、[外部MariaDBバックアップを利用した復元](backup-and-restore-gov/#restore-from-external)を参照してください。
...
> [注意]レプリケーション用アカウントがレプリケーション元(マスター) MariaDBに作成されている必要があります。
