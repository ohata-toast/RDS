...
웹 콘솔에서 서로 다른 2개의 파라미터 그룹을 선택한 후, **비교** 버튼을 클릭하면 파라미터가 무엇이 다른지 확인할 수 있습니다. 동일한 DB 엔진 버전뿐만 아니라 서로 다른 DB 엔진 버전의 파라미터 그룹도 비교할 수 있습니다.
...
웹 콘솔에서 파라미터 그룹을 선택한 후, **파라미터 편집** 버튼을 누르면 파라미터를 변경할 수 있습니다. 변경할 수 없는 파라미터는 값이 일반 텍스트로 나타나며, 변경할 수 있는 파라미터는 값을 변경할 수 있는 INPUT이 나타납니다. 편집 화면에서 `변경 사항 미리 보기` 버튼을 누르면 변경된 파라미터를 확인할 수 있는 별도 팝업 화면이 표시되며, `재설정` 버튼을 누르면 변경하기 전으로 되돌아갈 수 있습니다. 편집 모드에서 변경한 모든 값은 `변경 사항 저장` 버튼을 눌러야 파라미터 그룹에 반영됩니다. 변경된 파라미터 그룹의 DB 인스턴스 반영은 [파라미터 그룹 적용](./#_5) 항목을 참고합니다.